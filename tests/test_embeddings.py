...
@pytest.fixture
def mock_model():
    with patch('fuzzyshell.ModelHandler') as MockModelHandler:
        mock = MagicMock()
...
def test_search_ranking(fs_with_model, mock_model, temp_history):
    ...
    if docker_scores and non_docker_scores:
        assert min(docker_scores) > max(non_docker_scores), \
            "Docker-related commands should have higher scores than non-docker commands"
