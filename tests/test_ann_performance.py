...
from src.fuzzyshell.fuzzyshell import FuzzyShell, ANNSearchIndex


class TestANNPerformance(unittest.TestCase):
    ...
    def test_ann_vs_linear_search_performance(self):
        ...
        self.assertEqual(len(ann_results), len(linear_results))  # Same top_k
    ...
    def test_ann_clustering_overhead_measurement(self):
        ...
        # Fresh ANN index
        ann_index = ANNSearchIndex(n_clusters=32)
        ...
        self.assertLess(clustering_time, 2.0)  # Should complete within 2 seconds
    ...
    def test_ann_search_candidate_selection_efficiency(self):
        ...
        ann_index = ANNSearchIndex(n_clusters=16)
        ...
        candidates_1 = ann_index.search_candidates(query_embedding, n_candidates=1)
        candidates_3 = ann_index.search_candidates(query_embedding, n_candidates=3)
        candidates_5 = ann_index.search_candidates(query_embedding, n_candidates=5)
        ...
        self.assertLess(len(candidates_1), len(test_embeddings))
        self.assertLess(len(candidates_3), len(test_embeddings))
        self.assertLessEqual(len(candidates_1), len(candidates_3))  # More clusters = more candidates
        
        # Candidates should be valid indices
        for idx in candidates_3:
            self.assertGreaterEqual(idx, 0)
            self.assertLess(idx, len(test_embeddings))
    ...
    def test_ann_accuracy_vs_linear_search(self):
        ...
        self.assertGreater(recall, 0.5)  # At least 50% recall
...
class TestANNScalability(unittest.TestCase):
    ...
    def test_clustering_scalability(self):
        ...
        for size in dataset_sizes:
            ...
            # Measure clustering time
            ann_index = ANNSearchIndex(n_clusters=min(32, size // 10))  # Adaptive cluster count
            start_time = time.time()
            ann_index.fit(test_embeddings)
            clustering_time = time.time() - start_time
            ...
            self.assertLess(clustering_time, size * 0.01)  # Should be faster than 10ms per item
            self.assertTrue(ann_index.is_trained)
        ...
        for i in range(1, len(clustering_times)):
            if clustering_times[i-1] > 0:
                ...
                self.assertLess(scaling_factor, size_ratio)  # Better than quadratic
