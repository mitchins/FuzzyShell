...
from src.fuzzyshell.fuzzyshell import FuzzyShell, USE_ANN_SEARCH


class TestRealWorldSemanticSimilarity(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Mock model setup - we'll need to test with a real model
        self.mock_model = MagicMock()
        # We'll replace this with real model testing
    ...
    def test_command_description_semantic_similarity(self):
        ...
        if not hasattr(self.fuzzyshell, 'model') or self.fuzzyshell.model is None:
            self.skipTest("Model not available for semantic similarity testing")
        ...
        for command, description in test_pairs:
            ...
            similarity = np.dot(command_embedding, description_embedding) / (
                np.linalg.norm(command_embedding) * np.linalg.norm(description_embedding)
            )
        ...
        self.assertGreater(avg_similarity, 0.5, 
    ...
    def test_ann_search_with_real_commands(self):
        ...
        self.assertGreater(len(results), 0, "ANN search should return results")
        self.assertLessEqual(len(results), 10, "Should respect top_k limit")
        
        # Results should be real commands from our test set
        for result in results:
            command = result[0]
            self.assertIn(command, self.test_commands, 
    ...
    def test_command_complexity_handling(self):
        ...
        for command, description in complexity_tests:
            ...
            self.assertTrue(True)  # Placeholder - real test would check embeddings
...
class TestCommandCategoryRecognition(unittest.TestCase):
    ...
    def test_category_clustering_with_ann(self):
        ...
        if results:
            ...
            self.assertGreater(file_ops_in_top5, 0, 
